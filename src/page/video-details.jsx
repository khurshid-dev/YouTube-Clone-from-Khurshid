...
const VideoDetail = () => {
  ...
  useEffect(() => {
    fetchFromAPI(`videos?part=snippet,statistics&id=${id}`).then((id) => setVideoDetail(id.items[0]));
    fetchFromAPI(`search?part=snippet&relatedToVideoId=${id}&type=video`).then((id) => setSuggestedVideo(id.items));
    fetchFromAPI(`search?part=snippet&q=${id}`).then((id) => setVideos(id.items));
  }, [id]);
  ...
  return (
    <div className="max-w-[66%] xl:max-w-[80%] lg:max-w-[98%] md:max-w-[95%] flex md:flex-col justify-center gap-[2%] lg:gap-[1%] mx-auto mt-[1%]">
      <div data-aos="fade-up" data-aos-duration="1000" className="lg:w-[70%] md:w-full">
        {/* ============== Video Play ==========*/}
        <ReactPlayer url={`https://www.youtube.com/watch?v=${id}`} playing pip controls className="react-player" />

        {/* ============== Main Info ==========*/}
        <div data-aos="fade-up" data-aos-duration="1200" className="text-[20px] sm:text-[17px] font-semibold my-4">
          {title}
        </div>

        <div data-aos="fade-up" data-aos-duration="1500" className="flex justify-between">
          <div className="flex items-center basis-[25%] ss:basis-[35%] gap-2">
            <div className="w-full mr-[1%]">
              <img
                className="max-w-[40px] h-[40px] rounded-full"
            ...
            <div className="flex flex-col">
              ...
              <div className="text-[12px] w-[max-content] text-[rgb(171,171,171)]">
                {videoDetail?.statistics?.viewCount.slice(0, 2)} тыс. подписчиков
              </div>
            </div>
            <div className="btn-primary ss:hidden rounded-md ml-2 md:text-[12px]">
              <Button
                variant={`${subscrided ? "outlined" : "contained"}`}
                size="medium"
                onClick={() => setSubscrided((prev) => !prev)}
              >{`${subscrided ? "Подписано" : "Подписаться"}`}</Button>
            </div>
          </div>
          ...
          <div className="flex items-center ss:hidden">
            <div className="text-base ">
              <Button color="primary">
                <BiLike className="like-icons" /> {likeCount.slice(0, 3)} тыс.
              </Button>
            </div>
            <div className="text-base">
              <Button color="primary">
        ...
        {/* ============== Description ==========*/}
        <div data-aos="fade-up" data-aos-duration="2000" className="description-buttons flex flex-wrap gap-2 mt-6">
          <Button
            variant="outlined"
            sx={{
              borderRadius: "10px",
        ...
        <div className="description p-4  rounded-md w-full mt-3 mb-7 text-[14px] text-white">
          <p
            className={`tracking-[0.1px] mb-2 text-[13px] overflow-hidden 
                            ${descript ? "h-9" : "h-full"} transition-all`}
          >
            {description}
          </p>
          <div
            className="flex items-center justify-end gap-1 text-white font-bold text-[14px] cursor-pointer transition-all"
        ...
        <div className="md:hidden">
          {tags.slice(0, 8).map((item, idx) => (
            <a href="#" className="text-[#5e97e3]" key={idx}>
              #{item}
            </a>
          ))}
        </div>
      ...
      <div className="lg:w-[30%] md:hidden">
        <h1 className="font-bold text-[20px] mb-4">
          Похожие <span className="text-secondary">видео</span>
        </h1>
